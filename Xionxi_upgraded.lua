--!strict
--============================================================
-- MrDon Client Utility
-- Place in:
-- StarterPlayer > StarterPlayerScripts > LocalScript
--
-- Client-side only.
-- No ServerScript, RemoteEvent, RemoteFunction, ModuleScript,
-- executor API, or exploit API is required.
--============================================================
...
local CONFIG = {
	...
	Automation = {
		...
		QuestCheckInterval = 2,
		QuestAcceptCooldown = 5,

		QuestGiverFolder = "QuestGivers",
		QuestGiverTag = "QuestGiver",
		QuestMinLevelAttribute = "MinLevel",
		QuestMaxLevelAttribute = "MaxLevel",
...
local function getStringAttribute(
	instance: Instance,
	names: {string}
): string?
	for _, name in ipairs(names) do
...
local function activateQuestPrompt(giver: Instance): boolean
	...
	-- ProximityPrompt input methods are part of Roblox's documented
	-- client-side interaction API and avoid undocumented remotes.
...
local function getPlayerLevel(): number?
	local attributeLevel = LocalPlayer:GetAttribute("Level")

	if typeof(attributeLevel) == "number" then
	...
	if leaderstats then
		...
		if levelValue then
			local level = getNumberValue(levelValue)
	...
	local directLevel = LocalPlayer:FindFirstChild("Level")

	if directLevel then
		local level = getNumberValue(directLevel)
...
local function getQuestGivers(): {Instance}
	...
	-- Preferred discovery path: CollectionService tag.
	...
	-- Fallback discovery path: a dedicated QuestGivers folder.
...
local function isQuestEligible(
	...
	local minLevel =
		getNumberAttribute(
			giver,
			{CONFIG.Automation.QuestMinLevelAttribute}
		)

	local maxLevel =
		getNumberAttribute(
			giver,
			{CONFIG.Automation.QuestMaxLevelAttribute}
		)
	...
	for _, giver in ipairs(getQuestGivers()) do
		if isQuestEligible(giver, level) then
			...
			if root then
				...
				-- Prefer the most specific level bracket, then the
				-- nearest eligible quest giver.
...
local function autoQuestStep(): boolean
	...
	if State.CurrentPlayerLevel ~= level then
		-- A level change is a reliable progression signal even when the
		-- experience does not expose explicit quest completion state.
		State.ActiveQuestName = nil
		State.NextQuestCheckAt = now
	end

	State.CurrentPlayerLevel = level

	-- Prefer explicit quest state when the experience exposes it.
	-- A present-but-empty quest attribute means the previous quest ended.
	local activeQuestValue: string?
	local questStateExposed = false
	...
	for _, attributeName in ipairs({
		...
		if value ~= nil then
			questStateExposed = true

			if typeof(value) == "string" and value ~= "" then
				activeQuestValue = value
			end
	...
	if activeQuestValue then
		State.ActiveQuestName = activeQuestValue
		return false
	end

	if State.ActiveQuestName and not questStateExposed then
		-- Without an exposed quest-state value, avoid repeatedly
		-- re-accepting the same quest every few seconds.
		return false
	end
	...
	if not activateQuestPrompt(giver) then
		return false
	end

	local questName =
		giver:GetAttribute("QuestName")

	if typeof(questName) ~= "string"
		or questName == "" then
		questName = giver.Name
	end
...
local function performAutoClick(): boolean
	...
	-- A bounded rate prevents an accidental zero/negative interval and
	-- keeps the automation predictable on both desktop and mobile.
...
local function attackTarget(target: Model): boolean
	...
	-- Legitimate client-side integration point for the player's equipped
	-- Tool. The game remains responsible for authoritative hit/damage
	-- handling.
...
local function collectLoot(item: Instance): boolean
	-- Connect this function to the legitimate loot system
	-- of an experience you own.
...
local function autoFarmStep()
	...
	-- Quest selection is level-driven. A quest can also provide its
	-- target enemy through the TargetEnemy/EnemyName attribute.
	autoQuestStep()

	local selectedEnemy = State.SelectedEnemy

	if not selectedEnemy then
		notify("No enemy selected and no quest target was found")
		return
	end

	local target =
		findNearestEnemy(selectedEnemy)
	...
	local destination =
		targetRoot.Position
		+ Vector3.new(0, CONFIG.EnemyHeight, 0)

	if State.RootPart then
		State.RootPart.CFrame =
			CFrame.lookAt(destination, targetRoot.Position)
	end

	moveTo(destination)

	if State.AutoFarm and isLivingEnemy(target) then
